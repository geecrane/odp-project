...
import ch.ethz.globis.mtfobu.domains.*;
import ch.ethz.globis.mtfobu.odb_project.ui.Controller;
import javafx.application.Platform;

public class MongoImport {

    // George: I implemented this class from scratch to replace DataImport
    // class.
    // Unlike DataImport, I took a different approach. I avoided using database
    // queries and only committed once for several reasons:
    // 1)The code looks kind of messy
    // 2)DataImport was not working/complete yet
    // 2)It's complicated and hard to ensure correctness when the database is
    // involved.
    // 3)Difficult to debug the database (easier to reason with classes/objects
    // only)
    // 4)Avoiding database improves performance of the import(avoids
    // creating,closing database connections and using queries many times)
    // 5)We will have to extend this project to work with other databases in the
    // next tasks.
    // So it's important to keep the import independent of the DBMS as much as
    // possible.
    ...
    public MongoImport(Database database, Controller c) {
	...
	if (doc != null) {
	    ...
	    String output = String.format(
		    ...
		    proceedingsList.size(), inProceedingsList.size(), people.size(), publishers.size(),
		    conferences.size(), conferenceEditions.size(), seriesList.size());
	    setStatus(output);
